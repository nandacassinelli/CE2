...
double pontos, freqInicial, freqFinal, frequencia, Yn[MAX_NOS + 1][MAX_NOS + 2], g, variavelAtual[MAX_NOS], variavelProxima[MAX_NOS],
  ...
  while (fgets(linha, MAX_CHAR_LINHA, arquivo)) {
    ...
    if (elemento =='R' || elemento =='L' || elemento =='C') { // R, C, L <nome> <no1> <no2> <valor>
      ...
      netlist[nElementos - 1].cUmbc = 1e9;
        ...
        printf("%s %s %g %g %g\n", netlist[nElementos - 1].nome, escala, pontos, freqInicial, freqFinal);
        tem=1;
      }
